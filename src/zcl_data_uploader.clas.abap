...
.
            ...
            IF content  is NOT  initial.
              response->set_status( i_code = if_web_http_status=>ok
                                    i_reason = | { tablename } `ERROR EN JSON` | ).

              response->set_text( | { tablename } 'MM ERR0R EN JSON` | ).
            ENDIF.
        ...
        TRY.
            ...
            IF sy-subrc = 0.
              response->set_status( i_code = if_web_http_status=>ok
                                    i_reason = | { tablename } `dd Table updated successfully` | ).

              response->set_text( | { tablename } 'MM Table updated successfully` | ).
            ENDIF.
